...
class JarvisGUIExtended:
    ...
    def _start_animation(self):
        """Запуск анимаций через after() - безопасно для Tkinter"""
        def update_time():
            current_time = datetime.now().strftime("%H:%M:%S | %d.%m.%Y")
            self.time_label.config(text=current_time)
            self.root.after(1000, update_time)  # Обновление каждую секунду
        
        # Запустить через after вместо threading
    ...
    def _start_memory_analyzer(self):
        """Запуск анализа памяти через after() - безопасно"""
        def analyze_and_schedule():
            ...
            self.root.after(10000, analyze_and_schedule)
        
        # Первый запуск через 2 секунды после старта
        self.root.after(2000, analyze_and_schedule)
...
def launch_gui(jarvis=None):
    """Запуск расширенного GUI - ТОЛЬКО для отдельного запуска"""
    gui = JarvisGUIExtended(jarvis)
    
    if jarvis:
        jarvis.gui = gui
    
    # НЕ запускать в отдельном потоке!
    # GUI должен быть в главном потоке
    return gui

def launch_gui_async(jarvis=None):
    """Асинхронный запуск GUI (НЕ используется с asyncio JARVIS)"""
    import threading
    gui = JarvisGUIExtended(jarvis)
